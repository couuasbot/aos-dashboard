...
export function getEventLogPath(workspaceRoot) {
...
function normalizeEvent(e, idx) {
    ...
    if (!e.id)
        e.id = `legacy_${idx}`;
    if (!e.timestamp)
        e.timestamp = new Date(0).toISOString();
    if (e.type)
...
export async function readAutopilotLock(workspaceRoot) {
    ...
    if (obj && typeof obj === 'object') {
        if (Number.isFinite(obj.pid))
            pid = obj.pid;
        if (typeof obj.startTs === 'string')
            startTs = obj.startTs;
        if (Number.isFinite(obj.ttlMs))
            ttlMs = obj.ttlMs;
    }
    let stale = false;
    if (startTs && ttlMs) {
        ...
        if (Number.isFinite(start))
            stale = now > start + ttlMs;
    }
    return { path: lockPath, exists: true, pid, startTs, ttlMs, stale, ageMs };
}
export async function readEventsTail(workspaceRoot, limit = 200) {
    const fs = await import('node:fs/promises');
...
function applyEvent(tasks, e) {
    ...
    if (type === 'DISPATCH') {
        ...
        if (p.artifactsBaseDir)
            t.artifactsDir = p.artifactsBaseDir;
...
export async function getTasksState(workspaceRoot) {
...
export function computeMetrics(tasks) {
    const byState = {};
    const byLane = { execution: 0, ops: 0 };
    const byRole = {};
    const now = Date.now();
    const slaBreaches = [];
    for (const t of tasks) {
        ...
        if (t.state === 'In Progress' && t.inProgressAt) {
            ...
            if (ageMin > slaMinutes)
                slaBreaches.push({ taskId: t.taskId, ageMin, slaMinutes });
        }
    }
